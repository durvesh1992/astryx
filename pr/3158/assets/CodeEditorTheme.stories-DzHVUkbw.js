import{a1 as e,ah as T,G as j}from"./iframe-D8BQ-rJP.js";import{a as b,c as C,b as N,d as v,g as D,e as P,m as U,n as G,o as w,f as H,s as M,t as A,h as _,S as k}from"./presets-BhLX1qQK.js";import{C as S}from"./CodeEditor-BFa387Kx.js";import"./preload-helper-Ct5FWWRu.js";import"./highlightRanges-BcbbQgQG.js";const z=["import {useState, useEffect} from 'react';","","interface User {","  id: string;","  name: string;","  roles: string[];","}","",'const API_URL = "https://api.example.com";',"const MAX_RETRIES = 3;","","// Fetch user data with retry logic","async function fetchUser(id: string): Promise<User> {","  const response = await fetch(`${API_URL}/users/${id}`);","  if (!response.ok) {","    throw new Error(`HTTP ${response.status}`);","  }","  return response.json();","}","","export function UserCard({id}: {id: string}) {","  const [user, setUser] = useState<User | null>(null);","","  useEffect(() => {","    fetchUser(id).then(setUser);","  }, [id]);","","  if (!user) return <div>Loading...</div>;","","  return (",'    <div className="card">',"      <h2>{user.name}</h2>","      <span>{user.roles.length} roles</span>","    </div>","  );","}"].join(`
